...
public class CategoryDAO extends BaseHibernateDAO {
	...
	public Category findById(java.math.BigDecimal id) {
		log.debug("getting Category instance with id: " + id);
		try {
			Category instance = (Category) getSession().get(
					"com.spicy.domain.Category", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Category instance) {
		...
		try {
			List results = getSession().createCriteria(
					"com.spicy.domain.Category").add(Example.create(instance))
					.list();
